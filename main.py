...
def select_audio_device():
    ...
    while True:
        try:
            selection = input("Enter the ID of the input device to use (or press Enter for default): ")
            if not selection:
                return None
...
def main():
    ...
    if sr is None:
        try:
            ...
            print(f"Warning: Could not determine default sample rate: {e}", file=sys.stderr)
            sr = 44100
